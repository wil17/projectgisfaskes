...
class KlinikAdminController extends Controller
{
    ...
    {
        $query = Kliniks::select('id_klinik', 'nama_klinik', 'skala_usaha', 'alamat', 'kecamatan', 'kelurahan');
        ...
        $kecamatans = Kliniks::select('kecamatan')
            ->distinct()
            ->orderBy('kecamatan')
            ->pluck('kecamatan');
    ...
    public function search(Request $request)
    {
        ...
        $kliniks = Kliniks::where('nama_klinik', 'LIKE', "%{$query}%")
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            // Create record in kliniks table
            $klinik = Kliniks::create([
                'id_klinik' => $id,
            ...
            $this->logCreate($faskes, "Klinik baru '{$request->input('nama_klinik')}' telah ditambahkan ke sistem");
     ...
     *
     * @param  string  $id_klinik
     * @return \Illuminate\Http\Response
     */
    public function edit($id_klinik)
    {
        $klinik = Kliniks::where('id_klinik', $id_klinik)->firstOrFail();
        
        // Get related faskes data
        $faskes = Faskes::where('id', $klinik->id)->first();
        
        // If faskes exists, add latitude and longitude to klinik
        if ($faskes) {
            $klinik->latitude = $faskes->latitude;
            $klinik->longitude = $faskes->longitude;
        }
    ...
    public function update(Request $request, $id_klinik)
    {
        ...
        try {
            ...
            $allOldValues = array_merge($oldFaskesValues, $oldKlinikValues);
            
            // Refresh the model to get updated values
            $faskes->refresh();
            $this->logUpdate($faskes, $allOldValues, "Data klinik '{$request->input('nama_klinik')}' telah diperbarui");
...
public function destroy($id)
{
    DB::beginTransaction();
    try {
        $klinik = Kliniks::where('id_klinik', $id)->first();
        ...
        if ($faskes) {
            $this->logDelete($faskes, "Klinik '{$klinik->nama_klinik}' telah dihapus dari sistem");
        }
...
private function logExportActivity($description = '')
{
    try {
        ...
        $firstKlinik = Kliniks::first();
        $modelId = $firstKlinik ? $firstKlinik->id_klinik : 'export-pdf';
        ...
        $log->action = 'export';
        $log->model_type = 'App\Models\Kliniks';
